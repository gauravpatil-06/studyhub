...
import { BookOpen, Calendar, Search, RefreshCw, Trash2, User, FileText, ExternalLink, Download, Info, Clock, TrendingUp, ArrowUpDown } from 'lucide-react';
...
export const AdminMaterials = () => {
    ...
    const confirmDelete = async () => {
        ...
        try {
            await api.delete(`/admin/materials/${deletingId}`);
            setMaterials(materials.filter(m => m._id !== deletingId));
    ...
    const filteredMaterials = useMemo(() => {
        ...
        let result = materials.filter(m => m.userId && m.userId.name);

        // 1. Search Filter
        if (filters.searchTerm) {
            const query = filters.searchTerm.toLowerCase().trim();
            result = result.filter(m => {
                const studentName = (m.userId?.name || '').toLowerCase();
    ...
    return (
        <div className="space-y-6 pb-10 max-w-full mx-auto px-0">
            ...
            <PageHeader
                ...
                subtitle="Governance of all student-shared study documents and assets."
                right={
                    <div className="flex items-center gap-2">
                        ...
                        >
                            ...
                            <div className="relative z-10 w-full text-center sm:text-left">
                                <p className="text-[10px] font-bold text-white/90 tracking-wide">Total Shared Resources</p>
                                <div className="flex items-baseline gap-1.5 justify-center sm:justify-start">
                                    <span className="text-3xl font-black tabular-nums text-white leading-tight">{filteredMaterials.length}</span>
                                    <span className="text-[11px] font-black text-white/80 uppercase">Files</span>
                                </div>
                                <p className="text-[9px] font-semibold text-white/60 italic mt-0.5 whitespace-nowrap">Resources shared by students</p>
                            </div>
                        </motion.div>
                        <button
            ...
            <div className="flex lg:hidden justify-end mb-4">
                <div className="flex items-center gap-3 px-6 py-3 bg-[#47C4B7] rounded-xl shadow-lg border border-transparent hover:border-white/50 transition-all cursor-default relative overflow-hidden w-full max-w-[240px]">
                    ...
                    <div className="relative z-10 w-full">
                        <p className="text-[10px] font-bold text-white/90">Total Shared Resources</p>
                        <div className="flex items-baseline gap-1.5">
                            <span className="text-3xl font-black tabular-nums text-white">{filteredMaterials.length}</span>
                            <span className="text-[11px] font-black text-white/80 uppercase">Files</span>
                        </div>
                        <p className="text-[9px] font-semibold text-white/60 italic mt-0.5">Resources shared by students</p>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white dark:bg-gray-900 border border-gray-300 dark:border-white/20 rounded-3xl shadow-sm overflow-hidden overflow-x-auto custom-scrollbar">
                <table className="w-full text-left border-collapse min-w-[1500px] border-hidden">
                    <thead className="text-center">
                        <tr className="bg-[#47C4B7]/10 dark:bg-[#47C4B7]/5 border-b-2 border-gray-800/20 dark:border-white/40">
                            ...
                            <th className="px-5 py-5 text-[11px] font-black text-gray-800 dark:text-gray-200 uppercase tracking-widest whitespace-nowrap text-center border-r border-gray-800/20 dark:border-white/40">Title</th>
                            ...
                            <th className="px-5 py-5 text-[11px] font-black text-gray-800 dark:text-gray-200 uppercase tracking-widest whitespace-nowrap text-center">Action</th>
                        </tr>
                    </thead>
                        ...
                        <AnimatePresence mode='popLayout'>
                            {filteredMaterials.map((m, i) => (
                                ...
                                >
                                    ...
                                    <td className="px-6 py-5 text-center border-r border-gray-800/20 dark:border-white/40">
                                        <span className="text-xs font-black text-gray-800 dark:text-gray-200">{m.title}</span>
                                    </td>
                                    <td className="px-6 py-5 text-center border-r border-gray-300 dark:border-white/20 max-w-[250px]">
                                        <span className="text-[11px] font-bold text-gray-500 dark:text-gray-400 whitespace-normal break-words line-clamp-3">{m.description || 'N/A'}</span>
                                    </td>
                                    <td className="px-6 py-5 text-center border-r border-gray-800/20 dark:border-white/40">
                                        <a
                ...
                {filteredMaterials.length === 0 && (
                    <div className="py-24 text-center">
                        ...
                        <p className="text-gray-400 font-bold italic text-sm tracking-tight">No study materials discovered matching your search.</p>
                    </div>
                )}
            </div>
        </motion.div>
